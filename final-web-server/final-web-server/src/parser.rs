...
pub mod dep {
	...
	pub fn get_response(request_header: &str) -> Vec<u8> {
		let status_line: &str;
		let contents: ReadResult;

		let mut request = request_header.split(" ");
...
pub mod request_line {
	...
	pub fn get_target(request: Request) -> ReadResult {
		use format as f;
		let fs_prefix = "../pages";
		let mut full_request_path: String = f!("{fs_prefix}{}", request.path);
		...
		if fs::exists(&full_request_path).is_ok() {
			// think about what .is_ok() does
			let file_contents = if request.path.eq("/") {
				// TODO! make work with ./dir/
				// (index.html)
